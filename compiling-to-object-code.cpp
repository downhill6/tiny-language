...
namespace {
    ...
    /// 表示 var/in 表达式，包括变量列表和表达式体等信息。
    class VarExprAST : public ExprAST {
        // VarNames: 变量列表，Body: 表达式主体
...
static std::unique_ptr<ExprAST> ParseVarExpr() {
    ...
    while (true) {
        ...
        // 如果当前 token 不是 ','，说明变量列表已经解析完毕，退出循环
        if (CurTok != ',')
        break;
        getNextToken(); // eat the ','.
...
Value *VariableExprAST::codegen() {
    ...
    // 加载 value, load 指令用于从指定的地址中读取数据，返回读取的数据。
...
Value *BinaryExprAST::codegen() {
    ...
    // 特殊处理赋值操作符 '='，因为我们不希望将 LHS 作为表达式发出。
    if (Op == '=') {
        // 赋值操作符要求 LHS 是一个标识符。
        // 这假设我们在没有 RTTI 的情况下构建，因为 LLVM 默认情况下是这样构建的。
        // 如果使用 RTTI 构建 LLVM，则可以将其更改为 dynamic_cast 以进行自动错误检查。
        ...
        // 存储 value, store 指令用于将数据存储到指定的地址中。
...
Value *ForExprAST::codegen() {
    ...
    // 在函数的入口块中创建 alloca 指令，用于保存循环变量的值
...
Value *VarExprAST::codegen() {
    ...
    // 遍历 VarNames，为每个变量生成 alloca 指令，并将其保存到符号表中
    for (unsigned i = 0, e = VarNames.size(); i != e; ++i) {
        const std::string &VarName = VarNames[i].first;
        ExprAST *Init = VarNames[i].second.get();

        // 先生成变量的初始值，然后再将变量添加到符号表中
        // 这样可以防止初始值引用变量本身，也可以允许初始值引用之前定义的变量。
        // like this:
        //  var a = 1 in
    ...
    // 生成函数体的代码，现在所有的变量都在作用域中了。
    Value *BodyVal = Body->codegen();
    if (!BodyVal)
        return nullptr;

    // 恢复旧的变量绑定, 以便在退出 var 表达式的作用域之后，变量的值仍然是正确的。
    for (unsigned i = 0, e = VarNames.size(); i != e; ++i)
...
int main() {
    ...
    // 当前机器的 target triple https://clang.llvm.org/docs/CrossCompilation.html#target-triple
    ...
    // 如果找不到目标平台，则打印错误信息并退出。
    // 这通常发生在我们忘记初始化 TargetRegistry 或者我们有一个虚假的目标三元组。
    if (!Target) {
        errs() << Error;
        return 1;
    }

    // 使用通用 CPU 和特性字符串创建 TargetMachine 对象
    auto CPU = "generic";
    auto Features = "";
